<div class="mdl-layout__tab-panel" id="findings_panel">
        ...
        <div class="box-content">
            <div class="mdl-grid">
                <div class="mdl-cell mdl-cell--10-col mdl-cell--6-col-tablet mdl-cell--2-col-phone field-row mdl-zebra__desktop mdl-zebra__phone">
                    <i id="icon-emergency_mode" class="material-icons radio-icon"></i>
                ...
                <div class="mdl-cell mdl-cell--10-col mdl-cell--6-col-tablet mdl-cell--2-col-phone field-row">
                    <i id="icon-mil_on" class="material-icons radio-icon"></i>
                    <label>MIL on</label>
                </div>
                ...
                <div class="mdl-cell mdl-cell--12-col mdl-cell--8-col-tablet mdl-cell--4-col-phone field-row mdl-zebra__desktop mdl-zebra__phone">
                    <i id="icon-complaint_mechanic" class="material-icons"></i>
                ...
                <div class="mdl-cell mdl-cell--10-col mdl-cell--6-col-tablet mdl-cell--2-col-phone field-row">
                    <i id="icon-product_feature" class="material-icons radio-icon"></i>
                    <label>Product feature?</label></div>
                <div class="mdl-cell mdl-cell--2-col mdl-textfield--align-right"><label class="mdl-radio mdl-radio--large mdl-js-radio mdl-js-ripple-effect" for="product_feature_5f19d87769715">
                        <input value="1" class="mdl-radio__button saveonchange hascomment" large="1" name="product_feature" type="radio" id="product_feature_5f19d87769715">
                        <span class="mdl-radio__label">Yes</span>
                    </label>
                    <label class="mdl-radio mdl-radio--large mdl-js-radio mdl-js-ripple-effect" for="product_feature_5f19d8776975c"><input value="0" class="mdl-radio__button saveonchange hascomment" large="1" name="product_feature" type="radio" id="product_feature_5f19d8776975c"><span class="mdl-radio__label">No</span></label></div>
                <div class="mdl-cell mdl-cell--12-col field-row product_feature_comment comment-field-closed">
                    <i id="icon-follow_up" class="material-icons"></i>
                    <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label  fullwidth  form-group">
    ...
    <script>
        $(document).on("click", "body", function() {
            var findings_input = $('.findings_input').filter(function(input) {
                return $(this).val() == "";
            }).length;

            var emergency_mode = $(':radio[name="emergency_mode"]:not(:checked)').length - 1;
            var mil_on = $(':radio[name="mil_on"]:not(:checked)').length - 1;
            var product_feature = $(':radio[name="product_feature"]:not(:checked)').length - 1;

            var count = findings_input + emergency_mode + mil_on + product_feature;

            if (count != 0) {
                $("#Findings_badge").html("<span class='mdl-badge' id='Findings_badge' data-badge='" + count + "'></span>");
            } else $("#Findings_badge").html("<span></span>");

            if (emergency_mode == 1) {
                $("#icon-emergency_mode").html("<i class='material-icons error'>fiber_manual_record</i>");
            } else {
                $("#icon-emergency_mode").html("<i class='material-icons success'>check</i>");
            }
            if (mil_on == 1) {
                $("#icon-mil_on").html("<i class='material-icons error'>fiber_manual_record</i>");
            } else {
                $("#icon-mil_on").html("<i class='material-icons success'>check</i>");
            }

            if (product_feature == 1) {
                $("#icon-product_feature").html("<i class='material-icons error'>fiber_manual_record</i>");
            } else {
                $("#icon-product_feature").html("<i class='material-icons success'>check</i>");
            }
    ...
    <script>
     $(document).ready(function() {
            if (document.getElementById("complaint_mechanic").value == "") {
                $("#icon-complaint_mechanic").html("fiber_manual_record");$("#icon-complaint_mechanic").addClass("error");
                $("#icon-complaint_mechanic").removeClass("success");
            } else {
                $("#icon-complaint_mechanic").html("check");$("#icon-complaint_mechanic").addClass("success");
                $("#icon-complaint_mechanic").removeClass("error");
            }
     });
        
        //function Value Chnage
        $(document).on('keyup', '#complaint_mechanic', function() {
    ...
    <script>
     $(document).ready(function() {
            if (document.getElementById("follow_up").value == "") {
                $("#icon-follow_up").html("fiber_manual_record");$("#icon-follow_up").addClass("error");
                $("#icon-follow_up").removeClass("success");
            } else {
                $("#icon-follow_up").html("check");$("#icon-follow_up").addClass("success");
                $("#icon-follow_up").removeClass("error");
            }
     });
        
        //function Value Chnage
        $(document).on('keyup', '#follow_up', function() {
...
</div><?php /**PATH C:\xampp\htdocs\mbdiag\CodeForIntegration\Deepak\Mdiag_Demo\7thsept\resources\views/Diagnosis/EditTabs/Findings.blade.php ENDPATH**/ ?>